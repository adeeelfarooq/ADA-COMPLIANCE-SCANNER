const howitworkslists = [
  {
    Caption: "Enter Website URL",
    trans: "md:translate-y-10 md:-translate-x-10 translate-y-10 -translate-x-5",
    image:"weurl",
    rotation: "md:rotate-[-8deg] rotate-0 max-md:scale-87 max-md:-translate-x-2  ",
    
  },
  {
    Caption: "Automated Scan",
    image:"scanw",
    trans: "translate-x-1 translate-y-4 md:scale-100 scale-90 ",
    rotation: "md:rotate-[8deg] rotate-0 md:scale-110 scale-110",
  },
  {
    Caption: "Get Report  ",
    image:"report2",
     trans: "md:-translate-x-10 md:-translate-y-2 max-md:scale-105   -translate-x-5 translate-y-3 ",
    rotation: "md:rotate-[-8deg]  rotate-0 mt-10 max-md:scale-90 ",
  },
 
];

const accessibilityStats = [
  { label: "Alt Text Coverage", amount: "95%" },
  { label: "Color Contrast Ratio", amount: "4.5:1 " },
  { label: "Heading Structure", amount: "Properly Nested" },
  { label: "Keyboard Navigation", amount: "Accessible" },
  { label: "ARIA Labels", amount: "3 Missing" },
];

const cards = [
  {
    src: "/videos/scanner.mp4",
    
    name: "Madison",
    img: "/images/p1.png",
    translation: "md:translate-y-[-5%] translate-y-[-60%] ",
  },
  
];

export { howitworkslists, accessibilityStats, cards };